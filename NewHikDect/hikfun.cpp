...
void CALLBACK g_RealDataCallBack_V30(LONG lRealHandle, DWORD dwDataType, BYTE *pBuffer, DWORD dwBufSize, void* dwUser)
{
	...
	case NET_DVR_SYSHEAD: //系统头
		...
		if (!PlayM4_GetPort(&g_lPort))  //获取播放库未使用的通道号
		{
			break;
		}
		//m_iPort = g_lPort; //第一次回调的是系统头，将获取的播放库port号赋值给全局port，下次回调数据时即使用此port号播放
		if (dwBufSize > 0)
		{
			...
			if (!PlayM4_OpenStream(g_lPort, pBuffer, dwBufSize, 1024 * 1024)) //打开流接口
			{
				break;
			}
			...
			if (!PlayM4_Play(g_lPort, hWnd)) //播放开始
			{
				break;
			}
		}
